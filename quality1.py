...
for i in range(len(ids)):
    dirt = 'C:\\Users\\aungkon\\Desktop\\jhu-pilot\\' + str(ids[i]) + '\\rip_temp.mat'
    ...
    # with open(str(ids[i])+ "date_rip.json", 'w') as f:
    #     json.dump(date, f)
    with open(str(ids[i])+ "date_rip.json", 'r') as f:
        date = json.load(f)
    ...
    bar = Bar(df,
              ...
              color=color(columns='medal', palette=['Green', 'Red'],
                          sort=False),
              legend='top_left',
              title="Respiration data quality of participant id " + str(ids[i]),width = 800)

    output_file('Respiration_DataQuality_of_'+str(ids[i])+".html", title="RIP_BAR_Plot")
